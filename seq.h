 ...
 *     insert( pos, x )    Insert x at the index specified
...
public:
  ...
  void remove() {
    if (numElements == 0) {
      std::cerr << "remove: Tried to remove element from empty sequence\n";
      exit(-1);
    }
  ...
  T & operator [] ( int i ) const {
    if (i >= numElements || i < 0) {
      std::cerr << "element: Tried to access an element beyond the range of the sequence: "
...
template<class T>
void 
seq<T>::insert(int pos, const T &x )

{
  ...
  // Store the element at the  position
